import config from '../config/index.js';

const en = {
  __ERROR_MISSING_ENV: (v) => `Missing environment variable: ${v}`,
  __ERROR_MAX_GROUPS_REACHED: 'Maximum groups reached',
  __ERROR_MAX_USERS_REACHED: 'Maximum users reached',
  __COMMAND_ACTIVATE_LABEL: 'Activate',
  __COMMAND_ACTIVATE_TEXT: 'Activate',
  __COMMAND_ACTIVATE_REPLY: 'Activated',
  __COMMAND_ADVISE_LABEL: 'Advise',
  __COMMAND_ADVISE_TEXT: 'Advise',
  __COMMAND_ANALYZE_LABEL: 'Analyze',
  __COMMAND_ANALYZE_TEXT: 'Analyze',
  __COMMAND_APOLOGIZE_LABEL: 'Apologize',
  __COMMAND_APOLOGIZE_TEXT: 'Apologize',
  __COMMAND_BLAME_LABEL: 'Blame',
  __COMMAND_BLAME_TEXT: 'Blame',
  __COMMAND_COMPLAIN_LABEL: 'Complain',
  __COMMAND_COMPLAIN_TEXT: 'Complain',
  __COMMAND_DEACTIVATE_LABEL: 'Deactivate',
  __COMMAND_DEACTIVATE_TEXT: 'Deactivate',
  __COMMAND_DEACTIVATE_REPLY: 'Deactivated',
  __COMMAND_TALK_LABEL: 'Talk',
  __COMMAND_TALK_TEXT: 'Talk',
  __COMMAND_TALK_DEMO_LABEL: 'Talk',
  __COMMAND_TALK_DEMO_TEXT: 'Talk me about yourself',
  __COMMAND_COMFORT_LABEL: 'Comfort',
  __COMMAND_COMFORT_TEXT: 'Comfort',
  __COMMAND_COMMAND_LABEL: 'Command',
  __COMMAND_COMMAND_TEXT: 'Command',
  __COMMAND_CONTINUE_LABEL: 'Continue',
  __COMMAND_CONTINUE_TEXT: 'Continue',
  __COMMAND_DOC_LABEL: 'Documentation',
  __COMMAND_DOC_TEXT: 'Documentation',
  __COMMAND_DRAW_LABEL: 'Draw',
  __COMMAND_DRAW_TEXT: 'Draw',
  __COMMAND_DRAW_DEMO_LABEL: 'Draw',
  __COMMAND_DRAW_DEMO_TEXT: 'Draw a cat',
  __COMMAND_LAUGH_LABEL: 'Laugh',
  __COMMAND_LAUGH_TEXT: 'Laugh',
  __COMMAND_RESTART_LABEL: 'Restart',
  __COMMAND_RESTART_TEXT: 'Restart',
  __COMMAND_RESTART_REPLY: 'Restarting',
  __COMMAND_SUMMARIZE_LABEL: 'Summarize',
  __COMMAND_SUMMARIZE_TEXT: 'Summarize',
  __COMMAND_SUMMON_DEMO_LABEL: 'Summon',
  __COMMAND_SUMMON_DEMO_TEXT: `${config.BOT_NAME} What's up?`,
  __COMMAND_VERSION_LABEL: 'Version',
  __COMMAND_VERSION_TEXT: 'Version',
  __TEMPLATE_TITLE_COMMAND: 'Commands',
  __COMPLETION_INIT_MESSAGE: 'Hello!',
  __COMPLETION_PROMPT_ADVISE: 'Please summarize the following content briefly and advise appropriately.',
  __COMPLETION_PROMPT_ANALYZE: 'Please summarize the following content briefly and analyze properly.',
  __COMPLETION_PROMPT_APOLOGIZE: 'Please summarize the following content briefly and apologize sincerely.',
  __COMPLETION_PROMPT_BLAME: 'Please summarize the following content briefly and blame strongly.',
  __COMPLETION_PROMPT_COMFORT: 'Please summarize the following content briefly and comfort warmly.',
  __COMPLETION_PROMPT_COMPLAIN: 'Please summarize the following content briefly and complain gently.',
  __COMPLETION_PROMPT_LAUGH: 'Please summarize the following content briefly and laugh rudely.',
  __COMPLETION_PROMPT_SUMMARIZE: 'Please summarize the following content and provide some details.',
  __COMPLETION_QUOTATION_MARK_OPENING: '"',
  __COMPLETION_QUOTATION_MARK_CLOSING: '"',
  __USER_DISPLAY_NAME_SOMEONE: 'Someone',
};

export default en;
